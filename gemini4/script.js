...
// Ваш API-ключ NewsAPI.org.
// !!! ВАЖЛИВО: Замініть 'ВАШ_NEWS_API_КЛЮЧ_ТУТ' на ваш реальний ключ API NewsAPI.org.
// Зверніть увагу, що безкоштовний план NewsAPI.org може мати обмеження та проблеми з CORS.
const newsApiKey = 'b041db780ab646cdb065c74ddad8eb6f';
...
const newsFeedContainer = document.getElementById('news-feed'); // Новий елемент для новин
...
/**
 * Отримує останні новини з NewsAPI.org.
 */
async function fetchNews() {
    ...
    const newsApiUrl = `https://newsapi.org/v2/top-headlines?country=ua&apiKey=${newsApiKey}&pageSize=6`; // Новини з України, 6 статей

    try {
        const response = await fetch(newsApiUrl);
        if (!response.ok) {
            throw new Error(`Помилка отримання новин: ${response.statusText}`);
        }
        const data = await response.json();
        displayNews(data.articles);
    } catch (error) {
        newsFeedContainer.innerHTML = `<p class="text-red-600 text-lg text-center col-span-full">Не вдалося завантажити новини: ${error.message}. Перевірте ваш NewsAPI ключ та дозволи CORS.</p>`;
...
function displayNews(articles) {
    ...
    articles.forEach(article => {
        ...
        const source = article.source.name || 'Невідоме джерело';
        ...
        newsCard.innerHTML = `
            ...
            <p class="text-xs text-gray-500 mb-2">Джерело: ${source}</p>
