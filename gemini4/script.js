...
// URL публічного CORS-проксі. Ви можете замінити його на власний, якщо виникають проблеми.
...
async function fetchRssNews() {
    ...
    const fullRssUrl = `${corsProxyUrl}${encodeURIComponent(rssFeedUrl)}`;

    try {
        const response = await fetch(fullRssUrl);
        if (!response.ok) {
            throw new Error(`Помилка отримання RSS-стрічки: ${response.status} ${response.statusText}`);
        }
        ...
        if (xmlDoc.getElementsByTagName('parsererror').length > 0) {
            ...
            throw new Error('Помилка парсингу RSS-стрічки. Можливо, недійсний XML.');
        }
